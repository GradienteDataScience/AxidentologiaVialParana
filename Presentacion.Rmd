...
accidentologia %>%
  ...
  ggplot(aes(reorder(Lesiones, cantidad), cantidad, fill=Lesiones)) +
...
ggplot(aux, aes(x = hora, y=n)) +
  ...
  coord_polar()+
  labs(x="", y="", title="Cantidad siniestros de acuerdo a la hora",  caption = "1er trimestre 2018")+
  theme(
...
datos %>%
  mutate(
    tipo_lesion_fallecidos = factor(tipo_lesion_fallecidos, levels= c("Sin lesiones", "Lesiones leves", "Lesiones graves", "Lesiones graves_f"))
  )%>%
...
## Siniestro judicializado vs posesión de carnet
```{r}
(table(datos$judicializado,datos$carnet) %>%
  chisq.test(tabla0))$stdres%>%
corrplot(
    is.cor = FALSE, 
    method='circle', 
    #order="hclust", 
    title="Siniestro judicializado vs posesión de carnet", 
    addCoef.col = "black", # Add coefficient of correlation
    # Combine with significance
    #sig.level = 0.05, insig = "blank"#, 
    # hide correlation coefficient on the principal diagonal
